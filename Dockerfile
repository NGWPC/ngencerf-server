...
RUN set -eux && \
    PREBUILT_DIR="/ngencerf/prebuilt" && \
    mkdir -p "$PREBUILT_DIR" && \
    cd "$PREBUILT_DIR" && \
    rm -rf forecast_forcing_templates && \
    \
    git clone --depth 1 --filter=blob:none --sparse \
        -b "${NGEN_FORCING_TAG}" https://github.com/${NGEN_FORCING_ORG}/ngen-forcing.git tmp-ngen-forcing && \
    cd tmp-ngen-forcing && \
    ...
    mv NextGen_Forcings_Engine_BMI/BMI_NextGen_Configs/config_templates \
        "$PREBUILT_DIR/forecast_forcing_templates" && \
    cd "$PREBUILT_DIR" && \
