...
RUN --mount=type=cache,target=/root/.cache/pip,id=pip-cache \
    set -eux && \
    echo $CACHE_BUST && pip install "git+https://github.com/${MSW_MGR_ORG}/nwm-msw-mgr.git@${MSW_MGR_REF}" && \
    echo $CACHE_BUST && pip install "git+https://github.com/${DATA_ASSIMILATION_ORG}/nwm-data-assimilation.git@${DATA_ASSIMILATION_REF}" && \
    pip cache purge
