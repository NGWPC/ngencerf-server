...
4. On the Download CLI page, download the CLI executable and follow the instructions displayed on the page.
...
```bash
ngencerf url add https://ngencerf.example.com
```
...
Developers who need to build the CLI from source can run the following script from the `cli` directory:
...
```bash
ngencerf unlock run_ids
```

**Arguments:**

- `run_ids`: A space-separated list of one or more calibration run IDs, or the name of a Markdown file produced by the `jobs` command.
