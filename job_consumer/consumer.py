...
from job_consumer.config import RABBITMQ_URL, RABBITMQ_QUEUE
...
class JobConsumer:
    ...
    def run(self) -> None:
        logger.info("Starting consumer for queue '%s'", RABBITMQ_QUEUE)

        with Connection(RABBITMQ_URL) as conn:
            # Dev/local convenience only.
            # In production, the queue should already exist.
            bound_queue = self.queue(conn.default_channel)
            bound_queue.declare()
