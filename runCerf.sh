...
#       NGEN_FORCING_BRANCH='xxxx'
       ...
       if pip show "swe_mapping" > /dev/null 2>&1; then
           ...
           # Package is not installed, install with dependencies
           pip install -e "git+https://gitlab.sh.nextgenwaterprediction.com/NGWPC/nwm-ngen/ngen-forcing.git@${NGEN_FORCING_BRANCH}#egg=swe_mapping&subdirectory=swe_mapping"
       fi
    else
