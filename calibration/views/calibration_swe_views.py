...
def get_or_create_swe_plots(run: ValidationRun, date: str, plot_dir: str) -> Dict[str, str]:
    ...
    # Get the appropriate worker name
    ...
    # Derive SWE file inputs from the validation run.
    ...
    if not (os.path.exists(sim_map_path) and os.path.exists(raw_map_path) and os.path.exists(lumped_map_path)):
        swe_args = [date, swe_csv, swe_netcdf, gpkg, sim_map_path, raw_map_path, lumped_map_path]
        logger.info(f"Calling run_swe.main with arguments: {swe_args}")
...
def get_swe_images_by_date(request: Request) -> Response:
    ...
    if validation_type == ValidationType.VALID_ITERATION:
        plot_dir = os.path.join(get_output_validation_iteration_plot_dir(run.calibration_run, run.iteration_num, run.worker_name), 'SWE')
    else:
        plot_dir = os.path.join(get_output_validation_plot_dir(run.calibration_run), 'SWE')
