...
def get_calibration_jobs_for_evaluation(request: Request) -> Response:
    ...
    jobs = get_jobs(request.user, include_validations=True, run_status=[StatusEnum.DONE, StatusEnum.FAILED, StatusEnum.SERVER_ERROR, StatusEnum.CANCELLED])
...
def get_footer(request: Request) -> Response:
    ...
    response = {"version": settings.VERSION, "date": settings.DATE, "commit_hash": settings.COMMIT_HASH, "contact_email": settings.CONTACT_EMAIL}
