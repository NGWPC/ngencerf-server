...
def run_validation_job_callback_local(validation_run: ValidationRun, future: Future):
    ...
    logger.info(
        f'Job end callback received for Validation Job {validation_run.id}, Calibration Job {validation_run.calibration_run.id}/{validation_run.calibration_run.owner.username}, type: {validation_run.validation_type}')

    try:
        if future.exception() is not None:
            logger.error(
                f"Exception occurred in Validation Job {validation_run.id}, Calibration Job {validation_run.calibration_run.id}/{validation_run.calibration_run.owner.username}: {future.exception()}")
