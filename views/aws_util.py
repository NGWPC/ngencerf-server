...
def download_s3(uri, save_dir):
    ...
    print('download_s3: downloading', bucket, key, 'to', save_dir)
    filename = key.split('/')[-1]
    local_file_path = os.path.join(save_dir, filename)
    s3_client = boto3.client('s3')
    s3_client.download_file(bucket, key, local_file_path)
    print(f'download_s3: {uri} downloaded to {local_file_path}')
...
def download_all_s3(uri, save_dir):
    ...
    bucket, key = parse_s3_uri(uri)
    print('download_all_s3: downloading', bucket, key, 'to', save_dir)
    ...
    for s3_file in s3_objects:
        ...
        local_file_path = os.path.join(save_dir, filename)
        print(f'download_all_s3: {uri} downloaded to {local_file_path}')
        s3_client.download_file(bucket, s3_file, local_file_path)
    print(f"Downloaded files to {save_dir}: {', '.join(os.listdir(save_dir))}")
